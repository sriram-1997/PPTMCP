...
import {
  ...
  quantizePtToEmuStep,
} from "../render/utils/units.js";

export type SlideIR = SlideProgramSpec & {
  __themeSpaceScale?: number[];
};

type CardStyleWithSlot = NonNullable<CardElement["style"]> & {
  paddingSlot?: number;
};
...
function quantizeInches(value: unknown): number | undefined {
...
function quantizePt(value: unknown): number | undefined {
  if (typeof value !== "number" || !Number.isFinite(value)) {
    return undefined;
  }
...
function resolveLegacyCardPaddingPt(
  padding: unknown,
  themeSpaceScale: number[]
): number | undefined {
...
function computeRegionRect(region: RegionConfig, grid: GridConfig): { x: number; y: number; width: number; height: number } {
...
export function layerBucketForElementType(type: string): number {
  if (type === "image") {
    return 1; // background
  }
...
function quantizeTextInsets(element: TextElement): void {
  if (!element.style) {
    return;
  }
...
function quantizeListInsets(element: ListElement): void {
  if (element.bullet) {
...
function normalizeCardPadding(element: CardElement, slideIndex: number, elementIndex: number, themeSpaceScale: number[]): void {
  if (!element.style) {
    return;
  }
  const style = element.style as CardStyleWithSlot;
  ...
  if (hasPaddingSlot) {
    const slot = style.paddingSlot as number;
    if (!Number.isInteger(slot) || slot < 0 || slot >= themeSpaceScale.length) {
      throw new Error(`Slide ${slideIndex + 1} element ${elementIndex + 1}: card_padding_ambiguous`);
    }
    style.paddingPt = themeSpaceScale[slot];
  } else if (!hasPaddingPt) {
    const legacyPaddingPt = resolveLegacyCardPaddingPt((style as Record<string, unknown>).padding, themeSpaceScale);
...
function quantizeTableInsets(element: TableElement): void {
  if (!element.style) {
    return;
  }
...
function quantizeCalloutGeometry(element: CalloutElement): void {
  if (element.anchor) {
...
function quantizeConnectorGeometry(element: ConnectorElement): void {
  if (element.start) {
...
function canonicalizeElement(
  ...
  if (element.type === "text") {
    quantizeTextInsets(element as TextElement);
    return;
  }
  if (element.type === "list") {
    quantizeListInsets(element as ListElement);
    return;
  }
  if (element.type === "card") {
    normalizeCardPadding(element as CardElement, slideIndex, elementIndex, themeSpaceScale);
    return;
  }
  if (element.type === "table") {
    quantizeTableInsets(element as TableElement);
    return;
  }
  if (element.type === "callout") {
    quantizeCalloutGeometry(element as CalloutElement);
    return;
  }
  if (element.type === "connector") {
    quantizeConnectorGeometry(element as ConnectorElement);
    return;
  }
...
export function canonicalizeIR(ir: SlideIR): SlideIR {
  const cloned = JSON.parse(JSON.stringify(ir ?? {})) as SlideIR;
  const themeSpaceScale =
    Array.isArray(cloned.__themeSpaceScale) && cloned.__themeSpaceScale.every((value) => typeof value === "number")
  ...
  cloned.slides.forEach((slide, slideIndex) => {
    ...
    if (slide.regions && isObject(slide.regions) && slide.grid) {
      for (const [regionName, rawRegion] of Object.entries(slide.regions)) {
        ...
        const region = rawRegion as RegionConfig & {
          __rect?: { x: number; y: number; width: number; height: number };
        };
        if (
    ...
    const sorted = slide.elements
      .map((element, index) => {
        const explicitId =
          typeof (element as { id?: unknown }).id === "string" && String((element as { id?: unknown }).id).trim().length > 0
