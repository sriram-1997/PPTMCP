...
export function prepareCalloutElement(args: {
  ...
  const candidates: CalloutBox["placement"][] =
    args.element.box.placement === "auto"
      ? ["ne", "nw", "se", "sw", "e", "w", "n", "s"]
  ...
  if (!box) {
    if (args.element.box.placement === "auto") {
      args.hardErrors.push(
        `Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: callout_no_feasible_placement`
      );
    } else {
      args.hardErrors.push(
        `Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: callout_box_outside_region`
      );
    }
  ...
  if (leaderRectResult?.error) {
    args.hardErrors.push(
      `Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: ${leaderRectResult.error}`
    );
  }
  ...
  const leaderStyle = leaderEnd
    ...
    : undefined;
  const leaderStartArrow =
    args.element.leader?.startArrow ?? leaderStyle?.startArrow ?? "none";
  const leaderEndArrow =
    args.element.leader?.endArrow ?? leaderStyle?.endArrow ?? "none";
...
export function renderCalloutElement(slide: any, shapeType: any, element: PreparedCalloutElement, theme: ConcreteTheme): void {
  if (element.leader) {
