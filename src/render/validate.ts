...
function textRhythmTargetForRegion(regionName: unknown): number {
...
function validateCardElementSchema(
  ...
  if (typeof element.style !== "undefined") {
    if (!isPlainObject(element.style)) {
      ...
      if (
        typeof style.paddingSlot !== "undefined" &&
  ...
  root.slides.forEach((rawSlide, slideIdx) => {
    ...
    slide.elements.forEach((rawElement, elementIdx) => {
      ...
      if (enterpriseMode && !ENTERPRISE_ALLOWED_ELEMENT_TYPES.has(element.type)) {
        errors.push(
          `${elemPrefix} enterprise_shape_not_allowed ('${element.type}' is not permitted in enterprise mode)`
        );
      }
      if (
      ...
      if (enterpriseMode && element.type === "list" && Array.isArray(element.items)) {
        ...
        element.items.forEach((rawItem, itemIdx) => {
          ...
          if (normalized.length > ENTERPRISE_MAX_BULLET_CHARS) {
            errors.push(
              `${elemPrefix} enterprise_bullet_char_limit_exceeded (item ${itemIdx + 1}, ${normalized.length} > ${ENTERPRISE_MAX_BULLET_CHARS})`
            );
          }
        });
      }
      if (enterpriseMode && element.type === "card" && Array.isArray(element.body)) {
        element.body.forEach((rawBody, bodyIdx) => {
          ...
          rawBody.items.forEach((rawItem, itemIdx) => {
            ...
            if (normalized.length > ENTERPRISE_MAX_BULLET_CHARS) {
              errors.push(
                `${elemPrefix} enterprise_bullet_char_limit_exceeded (body ${bodyIdx + 1} item ${itemIdx + 1}, ${normalized.length} > ${ENTERPRISE_MAX_BULLET_CHARS})`
              );
            }
          });
        });
    ...
    if (enterpriseMode && slideBulletCount > ENTERPRISE_MAX_BULLETS_PER_SLIDE) {
      errors.push(
        `${prefix} enterprise_bullet_density_exceeded (${slideBulletCount} > ${ENTERPRISE_MAX_BULLETS_PER_SLIDE})`
      );
    }
