...
function tokenizeRuns(runs: PreparedListTextRun[]): Token[] {
  const tokens: Token[] = [];
  runs.forEach((run) => {
...
function tokensToRuns(tokens: Token[]): PreparedListTextRun[] {
  const runs: PreparedListTextRun[] = [];
...
function wrapRuns(runs: PreparedListTextRun[], maxUnitsPerLine: number): PreparedListLine[] {
  ...
  const addWordToken = (word: Token, includeSpace: boolean) => {
    if (includeSpace) {
  ...
  for (const token of tokens) {
    ...
    chunks.forEach((chunk, idx) => {
      const chunkToken: Token = { text: chunk, bold: token.bold, isSpace: false };
...
function resolveBulletSize(size: "sm" | "md" | undefined, theme: ConcreteTheme): number {
...
function resolveBulletColor(value: "text" | "muted" | "primary" | undefined, theme: ConcreteTheme): string {
  if (value === "muted") {
...
function buildTextRuns(lines: PreparedListLine[]): PreparedListTextRun[] {
  const output: PreparedListTextRun[] = [];
...
export function renderListLayout(
  slide: any,
  shapeType: any,
  layout: PreparedListLayout,
  overrides?: { fontFace?: string; fontSize?: number; color?: string }
): void {
...
export function renderListElement(slide: any, shapeType: any, element: PreparedListElement): void {
