 ...
 */
export async function exportToPdf(pptxPath) {
...
`;
        const result = spawnSync("powershell.exe", ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command", psScript], { stdio: "pipe", timeout: 60000 });
    ...
    try {
        ...
        for (const loPath of libreOfficePaths) {
            try {
                ...
                execSync(`"${loPath}" --headless --convert-to pdf --outdir "${dir}" "${resolvedPptx}"`, { stdio: "pipe", timeout: 30000 });
