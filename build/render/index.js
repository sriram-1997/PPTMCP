...
async function renderPptmcp(args) {
    ...
    try {
        ...
        throw new Error(`Failed to parse spec JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
    const rawSpecObj = rawSpec && typeof rawSpec === "object" && !Array.isArray(rawSpec)
        ? rawSpec
        : {};
    ...
    if (enterpriseMode) {
        ...
        softOverflowEntries.forEach((entry) => {
            hardErrors.push(`Slide ${entry.slide_index} element ${entry.element_index}: enterprise_overflow_hard_fail (truncation disabled)`);
        });
    }
...
export const pptRenderer = {
    name: "render_pptmcp",
    description: "Render a deterministic PPTX from a Slide Program JSON specification with grid discipline and layout validation",
    parameters: {
        type: "object",
        properties: {
            spec_path: {
