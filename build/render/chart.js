...
function clipLineToRect(start, end, rect) {
    const x0 = start.x;
    const y0 = start.y;
    const x1 = end.x;
    const y1 = end.y;
...
export function prepareChartElement(args) {
    if (args.bbox.width <= 0 || args.bbox.height <= 0) {
        args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_data_invalid (chart region has non-positive size)`);
    }
    const resolvedChartType = args.element.chartType ?? args.element.content?.chartType ?? "";
    let categories = [];
    let seriesData = [];
    let title = args.element.title ?? args.element.content?.title;
    let options;
    let overlays;
    if (resolvedChartType === "line") {
        const data = args.element.data;
        if (!data || !Array.isArray(data.labels)) {
            args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_data_invalid (labels must be an array)`);
        }
        else {
            categories = data.labels.map((label) => String(label));
        }
        if (!data || !Array.isArray(data.series) || data.series.length === 0) {
            args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_no_series`);
        }
        else {
            data.series.forEach((series) => {
                const values = Array.isArray(series.values) ? series.values : [];
                if (values.length !== categories.length) {
                    args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_data_length_mismatch`);
                }
                const numericValues = [];
                values.forEach((value) => {
                    if (typeof value !== "number" || !Number.isFinite(value)) {
                        args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_invalid_value`);
                    }
                    else {
        ...
        if (args.element.overlays && (args.element.overlays.averageLine || args.element.overlays.trendLine)) {
            if (categories.length < 2) {
                args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_overlay_invalid`);
            }
            else {
                    ...
                    if (!Number.isFinite(minVal) || !Number.isFinite(maxVal)) {
                        ...
                        if (plotArea.width <= 0 || plotArea.height <= 0) {
                            ...
                            if (args.element.overlays.averageLine) {
                                ...
                                const clipped = clipLineToRect({ x: plotArea.x, y }, { x: plotArea.x + plotArea.width, y }, plotArea);
                                if (clipped) {
                                    overlayLines.push({
                                        kind: "average",
        ...
        if (!content) {
            args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: chart_data_invalid (content missing)`);
        }
        else {
...
export function renderChartElement(slide, shapeType, element, theme) {
