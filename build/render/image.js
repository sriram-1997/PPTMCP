...
function resolveSurfaceColor(slot, theme) {
    switch (slot) {
...
function isFullSlide(bbox) {
    return (Math.abs(bbox.x) <= EPSILON_INCHES &&
...
function buildMaskedSvg(args) {
    ...
    const overlay = args.overlay &&
        `<rect x="${cropX}" y="${cropY}" width="${cropW}" height="${cropH}" rx="${roundNumber(radius)}" ry="${roundNumber(radius)}" fill="#${args.overlay.color}" opacity="${args.overlay.opacity}"/>`;
    return [
        `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}">`,
        `<defs>`,
        `<clipPath id="clip"><rect x="${cropX}" y="${cropY}" width="${cropW}" height="${cropH}" rx="${roundNumber(radius)}" ry="${roundNumber(radius)}"/></clipPath>`,
        `</defs>`,
...
export function prepareImageElement(args) {
    ...
    if (!Number.isFinite(opacity) || opacity < 0 || opacity > 1) {
        args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: image_opacity_invalid`);
    }
    ...
    if ([crop.left, crop.right, crop.top, crop.bottom].some((value) => value < 0 || value > 1 || !Number.isFinite(value)) ||
        crop.left + crop.right >= 1 ||
        crop.top + crop.bottom >= 1) {
        args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: image_crop_invalid`);
    }
    const hasCrop = crop.left > 0 || crop.right > 0 || crop.top > 0 || crop.bottom > 0;
    const overlaySlot = typeof args.element.overlaySurfaceSlot === "string" ? args.element.overlaySurfaceSlot : undefined;
    ...
    if (typeof overlayOpacity === "number" && (overlayOpacity < 0 || overlayOpacity > 1)) {
        args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: image_overlay_invalid`);
    }
    ...
    if (borderRadiusPt < 0 || borderRadiusPt > maxRadius) {
        args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: image_border_radius_invalid`);
    }
    let svgData;
    if (opacity < 1 || hasCrop || borderRadiusPt > 0 || overlaySlot) {
        try {
            ...
            args.hardErrors.push(`Slide ${args.slideIndex + 1} element ${args.elementIndex + 1}: image_mask_failed`);
        }
    }
    return {
        kind: "image",
