...
function validateTextElementSchema(element, elemPrefix, strict, errors) {
    ...
    if (typeof style.verticalAlign !== "undefined" &&
...
function validateTableElementSchema(element, elemPrefix, strict, errors) {
    ...
    content.rows.forEach((row, rowIdx) => {
        ...
        if (expectedCols > 0 && row.length !== expectedCols) {
            errors.push(`${elemPrefix} Table row ${rowIdx + 1} has ${row.length} cells, expected ${expectedCols}`);
        }
        row.forEach((cell, cellIdx) => {
            ...
            if (typeof cell === "number" || typeof cell === "boolean") {
                errors.push(`${elemPrefix} Table cell r${rowIdx + 1}c${cellIdx + 1} must be string/object; convert numeric/boolean values to strings`);
                return;
            }
            ...
            if ("rowSpan" in cell || "colSpan" in cell) {
                errors.push(`${elemPrefix} table_cell_merge_not_supported at r${rowIdx + 1}c${cellIdx + 1} (rowSpan/colSpan)`);
            }
                    ...
                    if (strict) {
                        ...
                        for (const key of Object.keys(cellStyle)) {
                            if (!allowedCellStyleFields.has(key)) {
                                errors.push(`${elemPrefix} Table cell r${rowIdx + 1}c${cellIdx + 1} unknown style field '${key}'`);
                            }
                        }
                    }
                }
...
function validateChartElementSchema(element, elemPrefix, strict, errors) {
    ...
    content.dataSeries.forEach((rawSeries, seriesIdx) => {
        if (!isPlainObject(rawSeries)) {
            errors.push(`${elemPrefix} chart_data_invalid (series ${seriesIdx + 1} must be an object)`);
            return;
        }
        ...
        series.data.forEach((point, pointIdx) => {
            if (!Array.isArray(point) || point.length !== 2) {
                errors.push(`${elemPrefix} chart_data_invalid (series ${seriesIdx + 1} data point ${pointIdx + 1} must be [label, value])`);
                return;
            }
            const [label, value] = point;
            if (typeof label !== "string") {
                errors.push(`${elemPrefix} chart_data_invalid (series ${seriesIdx + 1} data point ${pointIdx + 1} label must be string)`);
            }
            if (typeof value !== "number") {
                errors.push(`${elemPrefix} chart_data_invalid (series ${seriesIdx + 1} data point ${pointIdx + 1} value must be number)`);
            }
            if (typeof label === "string") {
        ...
        if (seriesIdx === 0) {
            ...
            errors.push(`${elemPrefix} chart_data_invalid (series ${seriesIdx + 1} categories do not align with series 1)`);
        }
    });
    if (strict) {
...
function validateListElementSchema(element, elemPrefix, strict, errors, nested) {
...
function validateEdgeElementSchema(element, elemPrefix, strict, errors, nodeIds) {
    if ("x" in element || "y" in element || "w" in element || "h" in element) {
        errors.push(`${elemPrefix} absolute_position_forbidden`);
    }
    ...
    if (typeof element.strokeSlot !== "undefined" &&
...
function validateCardElementSchema(element, elemPrefix, strict, errors) {
    if ("x" in element || "y" in element || "w" in element || "h" in element) {
        errors.push(`${elemPrefix} absolute_position_forbidden`);
    }
...
function validateRegionAnchorSchema(anchor, elemPrefix, errors) {
...
function validateCalloutElementSchema(element, elemPrefix, strict, errors) {
    if ("x" in element || "y" in element || "w" in element || "h" in element) {
        errors.push(`${elemPrefix} absolute_position_forbidden`);
    }
...
function validateConnectorElementSchema(element, elemPrefix, strict, errors) {
    if ("x" in element || "y" in element || "w" in element || "h" in element) {
        errors.push(`${elemPrefix} absolute_position_forbidden`);
    }
...
function validateImageElementSchema(element, elemPrefix, strict, errors) {
    ...
    if (typeof element.overlaySurfaceSlot !== "undefined") {
        if (typeof element.overlaySurfaceSlot !== "string" ||
...
function validateIconElementSchema(element, elemPrefix, strict, errors) {
    ...
    if (typeof element.verticalAlign !== "undefined" &&
...
export function buildOverflowMessage(entry) {
