...
function isTextSlot(value) {
    return typeof value === "string" && TEXT_SLOTS.has(value);
}
function isStrokeSlot(value) {
    return typeof value === "string" && STROKE_SLOTS.has(value);
}
function validateTemplateAnchor(anchor, prefix, errors) {
...
function validateTemplatePlaceholder(placeholder, prefix, errors) {
    ...
    if (placeholder.styleSlots) {
        if (typeof placeholder.styleSlots !== "object") {
            ...
            const slots = placeholder.styleSlots;
...
function validateTemplateDefinition(template, strict, errors, context) {
        ...
        if (!isPlainObject(template.variants)) {
            ...
            for (const [variantName, variantDef] of Object.entries(template.variants)) {
                ...
                if ("placeholders" in variantDef) {
                    const placeholders = variantDef.placeholders;
...
function mergePlaceholders(base, child) {
    const result = base.map((placeholder) => ({ ...placeholder }));
    const index = new Map();
    ...
    for (const placeholder of child) {
        ...
        result[existingIndex] = {
            ...
            leader: basePlaceholder.leader || placeholder.leader
                ? { ...basePlaceholder.leader, ...placeholder.leader }
                : undefined,
            connectorStyle: basePlaceholder.connectorStyle || placeholder.connectorStyle
...
function mergeLayers(base, child) {
    if (!base && !child) {
        return undefined;
    }
    const result = [];
...
function mergeTemplates(base, child) {
    return {
        id: child.id,
...
function resolveTemplate(templateRef, strict, errors) {
...
function resolveTemplateById(templateId, strict, errors, stack) {
...
function applyVariant(template, variantName, strict, errors, context) {
    if (!variantName) {
        return template;
    }
    const prefix = `Template ${context}:`;
    ...
    const placeholderOverrides = variantDef.placeholders;
    ...
    for (const override of placeholderOverrides) {
        ...
        if (typeof overrideObj.paddingToken !== "undefined") {
            if (!Number.isInteger(overrideObj.paddingToken) || overrideObj.paddingToken < 0 || overrideObj.paddingToken > 5) {
                errors.push(`${prefix} template_variant_invalid`);
            }
            else {
                target.styleSlots = { ...target.styleSlots, padding: overrideObj.paddingToken };
            }
        }
        if (typeof overrideObj.gapToken !== "undefined") {
            if (!Number.isInteger(overrideObj.gapToken) || overrideObj.gapToken < 0 || overrideObj.gapToken > 5) {
                errors.push(`${prefix} template_variant_invalid`);
            }
            else {
                target.gapToken = overrideObj.gapToken;
            }
        }
        if (typeof overrideObj.styleSlots !== "undefined") {
            if (!isPlainObject(overrideObj.styleSlots)) {
                ...
                const slots = overrideObj.styleSlots;
                if (typeof slots.text !== "undefined" && !isTextSlot(slots.text)) {
                    errors.push(`${prefix} template_variant_invalid`);
                }
                if (typeof slots.stroke !== "undefined" && !isStrokeSlot(slots.stroke)) {
        ...
        if (typeof overrideObj.region !== "undefined") {
            if (!isPlainObject(overrideObj.region)) {
                ...
                const region = overrideObj.region;
...
function resolveSlot(overrideValue, placeholderValue, defaultValue) {
...
function resolveVariantValue(fill, placeholderVariant, prefix, errors) {
...
function resolveOverlaySurfaceSlot(value, prefix, errorCode, errors) {
...
function resolvePaddingSlot(slot, prefix, errors) {
...
function resolvePaddingInches(slot, theme, prefix, errors) {
    if (typeof slot === "undefined") {
        return undefined;
    }
...
function resolveGapInches(slot, theme, prefix, errors) {
    if (typeof slot === "undefined") {
        return undefined;
    }
...
function assertRectQuantized(rect, prefix, errors) {
    if (!isEmuAligned(rect.x) ||
        !isEmuAligned(rect.y) ||
...
function regionToRectInches(grid, region) {
...
function rectToRegion(grid, rect) {
    const { cellWidth, cellHeight } = gridMetrics(grid);
    const stepX = cellWidth + grid.gutter;
    const stepY = cellHeight + grid.gutter;
...
function resolveStrokeSlot(slot, prefix, errors) {
    if (typeof slot === "undefined") {
        return undefined;
    }
...
function resolveTextSlot(slot, prefix, errors) {
    if (typeof slot === "undefined") {
        return undefined;
    }
    if (!isTextSlot(slot)) {
...
function textScaleForSlot(theme, slot) {
    return theme.fontScale[TEXT_SLOT_MAP[slot]];
}
function strokeWidthForSlot(theme, slot) {
...
function toRegionAnchor(anchor) {
    return {
        type: "region",
...
function computeZ(placeholder, layers, errors, prefix) {
    let base = 0;
...
function ensureFillObject(fill, prefix, errors) {
...
function validateFillFields(fill, allowed, prefix, strict, errors) {
    if (!strict) {
        return;
    }
...
function validateItemFields(item, allowed, prefix, strict, errors) {
    if (!strict) {
        return;
    }
...
function pushCardFillError(prefix, errors, code = "template_card_fill_invalid") {
    errors.push(`${prefix} ${code}`);
}
function parseCardStyleSpec(style, prefix, errors) {
    const parsed = {};
    ...
    if (typeof style.accent !== "undefined") {
        ...
        parsed.accent = {
            edge: typeof accent.edge === "string" ? accent.edge : "none",
            color: typeof accent.color === "string" ? accent.color : undefined,
            width: typeof accent.width === "number" ? accent.width : undefined,
        };
    }
    return parsed;
}
function parseCardHeader(header, prefix, errors) {
...
function parseCardFooter(footer, prefix, errors) {
...
function parseCardBody(body, prefix, errors, strict) {
    ...
    body.forEach((entry, idx) => {
        ...
        for (const listEntry of item.items) {
            if (typeof listEntry === "string") {
                normalizedItems.push({ text: listEntry });
            }
            else if (isPlainObject(listEntry) && typeof listEntry.text === "string") {
                normalizedItems.push({ text: listEntry.text });
            }
            else {
                pushCardFillError(itemPrefix, errors);
                return;
        ...
        listItems.push({
            ...
            bullet: isPlainObject(item.bullet) ? item.bullet : undefined,
            indent: isPlainObject(item.indent) ? item.indent : undefined,
            lineGap: typeof item.lineGap === "number" ? item.lineGap : undefined,
...
export function compileTemplateSpec(args) {
    ...
    root.slides.forEach((rawSlide, slideIdx) => {
        ...
        let templateRef = slide.template;
        let templateVariant;
        if (isPlainObject(slide.template) && "name" in slide.template) {
            const templateObj = slide.template;
        ...
        const fills = isPlainObject(slide.fills) ? slide.fills : {};
        ...
        for (const placeholder of template.placeholders) {
            ...
            const paddingSlot = resolvePaddingSlot(resolveSlot(fillObjForPadding?.paddingSlot, placeholder.styleSlots?.padding, template.defaultSlots?.padding), placeholderPrefix, errors);
            const paddingIn = resolvePaddingInches(paddingSlot, args.theme, placeholderPrefix, errors);
            let contentRegionId = null;
            ...
            const itemCount = Array.isArray(fillObj?.items) ? (fillObj?.items).length : (typeof fill === "undefined" ? 0 : 1);
            ...
            const items = Array.isArray(fillObj?.items) ? fillObj?.items : null;
            ...
            if (items) {
                if (fillObj) {
                    validateFillFields(fillObj, new Set(["items", "variant"]), placeholderPrefix, args.strict, errors);
                }
                if (placeholder.kind === "connector") {
                ...
                if (layoutMode === "single") {
                    const hasNestedCard = items.some((item) => isPlainObject(item) && item.kind === "card");
                    if (hasNestedCard) {
                ...
                items.forEach((item, idx) => {
                    ...
                    if (!allowedKinds.includes(kindForAllow)) {
                        errors.push(`${itemPrefix} template_item_kind_invalid`);
                        return;
                    }
                    ...
                    if (kindValue === "text") {
                        validateItemFields(itemObj, new Set(["kind", "text", "items", "textStyleSlot", "paddingSlot", "variant"]), itemPrefix, args.strict, errors);
                        const textValue = typeof itemObj.text === "string" ? itemObj.text : null;
                        if (typeof textValue !== "string") {
                            errors.push(`${itemPrefix} template_item_field_invalid`);
                            return;
                        }
                        const textSlot = resolveTextSlot(resolveSlot(itemObj.textStyleSlot, placeholder.styleSlots?.text, template.defaultSlots?.text), itemPrefix, errors);
                    ...
                    if (kindValue === "list") {
                        validateItemFields(itemObj, new Set(["kind", "items", "style", "bullet", "indent", "lineGap"]), itemPrefix, args.strict, errors);
                        if (!Array.isArray(itemObj.items)) {
                            errors.push(`${itemPrefix} template_item_field_invalid`);
                            return;
                        }
                        const listItems = [];
                        for (const entry of itemObj.items) {
                            if (typeof entry === "string") {
                                listItems.push({ text: entry });
                            }
                            else if (isPlainObject(entry) && typeof entry.text === "string") {
                                listItems.push({ text: entry.text });
                            }
                            else {
                                errors.push(`${itemPrefix} template_item_field_invalid`);
                                return;
                        ...
                        elements.push({
                            type: "list",
                            region: regionId,
                            style: typeof itemObj.style === "string" ? itemObj.style : undefined,
                            bullet: isPlainObject(itemObj.bullet) ? itemObj.bullet : undefined,
                            indent: isPlainObject(itemObj.indent) ? itemObj.indent : undefined,
                    ...
                    if (kindValue === "card") {
                        ...
                        if (typeof itemObj.variant !== "undefined") {
                            ...
                            cardVariant = itemObj.variant;
                        }
                        const cardStyle = typeof itemObj.style === "undefined"
                            ? undefined
                            : isPlainObject(itemObj.style)
                                ? parseCardStyleSpec(itemObj.style, itemPrefix, errors)
                                : (pushCardFillError(itemPrefix, errors), null);
                        if (cardStyle === null) {
                            return;
                        }
                        const cardHeader = typeof itemObj.header === "undefined"
                            ? undefined
                            : isPlainObject(itemObj.header)
                                ? parseCardHeader(itemObj.header, itemPrefix, errors)
                                : (pushCardFillError(itemPrefix, errors), null);
                        if (cardHeader === null) {
                            return;
                        }
                        const cardFooter = typeof itemObj.footer === "undefined"
                            ? undefined
                            : isPlainObject(itemObj.footer)
                                ? parseCardFooter(itemObj.footer, itemPrefix, errors)
                                : (pushCardFillError(itemPrefix, errors), null);
                        if (cardFooter === null) {
                            return;
                        }
                    ...
                    if (kindValue === "table") {
                        validateItemFields(itemObj, new Set(["kind", "headers", "rows", "paddingSlot", "variant"]), itemPrefix, args.strict, errors);
                        if (!Array.isArray(itemObj.headers) ||
                        ...
                        elements.push({
                            type: "table",
                            region: regionId,
                            content: { headers: itemObj.headers, rows: itemObj.rows },
                            variant,
                            z,
                            style: Object.keys(style).length > 0 ? style : undefined,
                        });
                    ...
                    if (kindValue === "chart") {
                        validateItemFields(itemObj, new Set(["kind", "chartType", "title", "dataSeries", "variant"]), itemPrefix, args.strict, errors);
                    ...
                    if (kindValue === "image") {
                        validateItemFields(itemObj, new Set([
                            "kind",
                            "src",
                            "fit",
                            "variant",
                        ...
                        elements.push({
                            ...
                            crop: isPlainObject(itemObj.crop) ? itemObj.crop : undefined,
                            overlaySurfaceSlot: resolveOverlaySurfaceSlot(itemObj.overlaySurfaceSlot, itemPrefix, "template_item_field_invalid", errors),
                    ...
                    if (kindValue === "icon") {
                        validateItemFields(itemObj, new Set(["kind", "name", "sizeToken", "colorSlot", "align", "verticalAlign"]), itemPrefix, args.strict, errors);
                        ...
                        const colorSlot = resolveTextSlot(resolveSlot(itemObj.colorSlot, placeholder.styleSlots?.text, template.defaultSlots?.text), itemPrefix, errors);
                        if (!colorSlot) {
                            errors.push(`${itemPrefix} template_item_field_invalid`);
                            return;
                        }
                        ...
                        if (typeof itemObj.verticalAlign !== "undefined" &&
                        ...
                        elements.push({
                            ...
                            region: regionId,
                            name: itemObj.name,
                            sizeToken: itemObj.sizeToken,
                            colorSlot,
                            align: typeof itemObj.align === "string" ? itemObj.align : undefined,
                            verticalAlign: typeof itemObj.verticalAlign === "string" ? itemObj.verticalAlign : undefined,
                            z,
                        });
                        return;
                    }
                    if (kindValue === "callout") {
                        validateItemFields(itemObj, new Set(["kind", "text", "content", "textStyleSlot", "paddingSlot", "variant", "leader"]), itemPrefix, args.strict, errors);
                        ...
                        if (isPlainObject(itemObj.leader)) {
                            const leaderObj = itemObj.leader;
                            if (typeof leaderObj.startArrow !== "undefined" &&
                                !["none", "triangle"].includes(String(leaderObj.startArrow))) {
                                errors.push(`${itemPrefix} template_item_field_invalid`);
                                return;
                            }
                            if (typeof leaderObj.endArrow !== "undefined" &&
                        ...
                        const paddingSlot = resolvePaddingSlot(resolveSlot(itemObj.paddingSlot, placeholder.box.paddingSlot ?? placeholder.styleSlots?.padding, template.defaultSlots?.padding), itemPrefix, errors);
                        const textSlot = resolveTextSlot(resolveSlot(itemObj.textStyleSlot, placeholder.styleSlots?.text, template.defaultSlots?.text), itemPrefix, errors);
                        const textStyle = {};
                        if (textSlot) {
                            const scale = textScaleForSlot(args.theme, textSlot);
                        ...
                        const leader = placeholder.leader || itemObj.leader ? ({
                            style: "line",
                            endCap: "none",
                            startArrow: itemObj.leader?.startArrow ?? placeholder.leader?.startArrow,
                            endArrow: itemObj.leader?.endArrow ?? placeholder.leader?.endArrow,
                        }) : undefined;
                        elements.push({
                            type: "callout",
                            id: placeholder.id,
                            region: regionId,
                            anchor: toRegionAnchor(placeholder.anchor),
                            box: {
            ...
            if (placeholder.kind === "text") {
                ...
                validateFillFields(fillObj, new Set(["content", "textStyleSlot", "paddingSlot", "variant"]), placeholderPrefix, args.strict, errors);
                ...
                const textSlot = resolveTextSlot(resolveSlot(fillObj.textStyleSlot, placeholder.styleSlots?.text, template.defaultSlots?.text), placeholderPrefix, errors);
            ...
            if (placeholder.kind === "card") {
                ...
                const cardStyle = typeof fillObj.style === "undefined"
                    ...
                    : isPlainObject(fillObj.style)
                        ? parseCardStyleSpec(fillObj.style, placeholderPrefix, errors)
                        : (pushCardFillError(placeholderPrefix, errors), null);
                if (cardStyle === null) {
                    continue;
                }
                const cardHeader = typeof fillObj.header === "undefined"
                    ? undefined
                    : isPlainObject(fillObj.header)
                        ? parseCardHeader(fillObj.header, placeholderPrefix, errors)
                        : (pushCardFillError(placeholderPrefix, errors), null);
                if (cardHeader === null) {
                    continue;
                }
                const cardFooter = typeof fillObj.footer === "undefined"
                    ? undefined
                    : isPlainObject(fillObj.footer)
                        ? parseCardFooter(fillObj.footer, placeholderPrefix, errors)
                        : (pushCardFillError(placeholderPrefix, errors), null);
                if (cardFooter === null) {
                    continue;
                }
            ...
            if (placeholder.kind === "list") {
                ...
                validateFillFields(fillObj, new Set(["items", "style", "bullet", "indent", "lineGap"]), placeholderPrefix, args.strict, errors);
                if (!Array.isArray(fillObj.items)) {
                    errors.push(`${placeholderPrefix} template_fill_invalid`);
                    continue;
                }
                const listItems = [];
                for (const entry of fillObj.items) {
                    if (typeof entry === "string") {
                        listItems.push({ text: entry });
                    }
                    else if (isPlainObject(entry) && typeof entry.text === "string") {
                ...
                elements.push({
                    type: "list",
                    region: regionId,
                    style: typeof fillObj.style === "string" ? fillObj.style : undefined,
                    bullet: isPlainObject(fillObj.bullet) ? fillObj.bullet : undefined,
                    indent: isPlainObject(fillObj.indent) ? fillObj.indent : undefined,
            ...
            if (placeholder.kind === "table") {
                ...
                validateFillFields(fillObj, new Set(["content", "paddingSlot", "variant"]), placeholderPrefix, args.strict, errors);
            ...
            if (placeholder.kind === "image") {
                ...
                validateFillFields(fillObj, new Set([
                    "src",
                    "fit",
                    "variant",
                    "opacity",
                    ...
                    "overlayOpacity",
                    "borderRadiusPt",
                ]), placeholderPrefix, args.strict, errors);
                ...
                const element = {
                    ...
                    crop: isPlainObject(fillObj.crop) ? fillObj.crop : undefined,
                    overlaySurfaceSlot: resolveOverlaySurfaceSlot(fillObj.overlaySurfaceSlot, placeholderPrefix, "template_fill_invalid", errors),
            ...
            if (placeholder.kind === "icon") {
                ...
                validateFillFields(fillObj, new Set(["name", "sizeToken", "colorSlot", "align", "verticalAlign"]), placeholderPrefix, args.strict, errors);
                ...
                const colorSlot = resolveTextSlot(resolveSlot(fillObj.colorSlot, placeholder.styleSlots?.text, template.defaultSlots?.text), placeholderPrefix, errors);
                if (!colorSlot) {
                    errors.push(`${placeholderPrefix} template_fill_invalid`);
                    continue;
                }
                ...
                if (typeof fillObj.verticalAlign !== "undefined" &&
                ...
                const element = {
                    ...
                    region: regionId,
                    name: fillObj.name,
                    sizeToken: fillObj.sizeToken,
                    colorSlot,
                    align: typeof fillObj.align === "string" ? fillObj.align : undefined,
                    verticalAlign: typeof fillObj.verticalAlign === "string" ? fillObj.verticalAlign : undefined,
                    z,
                };
                elements.push(element);
                continue;
            ...
            if (placeholder.kind === "callout") {
                ...
                validateFillFields(fillObj, new Set(["text", "content", "textStyleSlot", "paddingSlot", "variant", "leader"]), placeholderPrefix, args.strict, errors);
                ...
                const paddingSlot = resolvePaddingSlot(resolveSlot(fillObj.paddingSlot, placeholder.box.paddingSlot ?? placeholder.styleSlots?.padding, template.defaultSlots?.padding), placeholderPrefix, errors);
                const textSlot = resolveTextSlot(resolveSlot(fillObj.textStyleSlot, placeholder.styleSlots?.text, template.defaultSlots?.text), placeholderPrefix, errors);
                const textStyle = {};
                if (textSlot) {
                    const scale = textScaleForSlot(args.theme, textSlot);
                ...
                const leader = placeholder.leader || fillObj.leader ? ({
                    style: "line",
                    endCap: "none",
                    startArrow: fillObj.leader?.startArrow ?? placeholder.leader?.startArrow,
                    endArrow: fillObj.leader?.endArrow ?? placeholder.leader?.endArrow,
                }) : undefined;
                const regionId = contentRegionId ?? placeholder.id;
                const element = {
                    type: "callout",
            ...
            if (placeholder.kind === "connector") {
                ...
                if (connectorFill) {
                    validateFillFields(connectorFill, new Set(["strokeSlot", "variant", "startArrow", "endArrow"]), placeholderPrefix, args.strict, errors);
                ...
                const strokeSlot = resolveStrokeSlot(resolveSlot(connectorFill?.strokeSlot, placeholder.styleSlots?.stroke, template.defaultSlots?.stroke), placeholderPrefix, errors);
                const style = {};
                if (strokeSlot) {
                ...
                const element = {
                    ...
                    start: toRegionAnchor(placeholder.start),
                    end: toRegionAnchor(placeholder.end),
                    style: Object.keys(style).length > 0 ? style : undefined,
                    variant,
                    z,
                };
    ...
    const compiledSpec = {
        title: root.title,
        metadata: isPlainObject(root.metadata) ? root.metadata : undefined,
        theme: root.theme,
        styleTokens: root.styleTokens,
