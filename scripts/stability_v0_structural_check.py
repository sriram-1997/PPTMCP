...
PPTX_LIGHT = "out/demos/pptmcp_capabilities_deck_v1.pptx"
PPTX_DARK = "out/demos/pptmcp_capabilities_deck_v1_dark.pptx"
OUT = "out/demos/pptmcp_capabilities_deck_v1.structural_check.json"
...
def main() -> int:
    out = {
        "light": check_pptx(PPTX_LIGHT),
        "dark": check_pptx(PPTX_DARK),
    }
    ...
    for key in ("light", "dark"):
        r = out[key]
        if not r.get("zip_open") or not r.get("presentation_xml") or not r.get("rels_present") or r.get("slide_xml_parse_errors"):
            ok = False
    if not ok:
